...
public class MapsActivity extends AppCompatActivity implements OnMapReadyCallback, GoogleMap.OnGroundOverlayClickListener {
    ...
    private GroundOverlay eGroundOverlay;
    ...
    public static final CameraPosition LSU =
            new CameraPosition.Builder().target(new LatLng(30.4133, -91.1800))
                    ...
                    .tilt(50)
                    .build();
    /** Manipulates the map once available. This callback is triggered when the map is ready to be used. This is where we can add markers or lines, add listeners or move the camera.
    ...
    public void onGroundOverlayClick(GroundOverlay groundOverlay) {
        //checks what lot was selected
        if(groundOverlay.getTag()=="central"){
            //will add or decrement based on buttons we add later
            centralAvail--;
            central.setSnippet(centralAvail+"/1264");
        }
        else if(groundOverlay.getTag()=="east"){
            eastAvail--;
            east.setSnippet(eastAvail+"/1275");
        }
    }
}
